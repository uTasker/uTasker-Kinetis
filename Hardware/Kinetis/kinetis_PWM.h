/***********************************************************************
    ...
    *********************************************************************
    04.01.2016 Added DMA buffer to PWM support                           // {1}
    05.01.2016 Added optional PWM cycle interrupt                        // {2}

*/


#if defined _PWM_CODE                                                    // {2}
/* =================================================================== */
...
static __interrupt void _PWM_Interrupt_0(void)
{
    FTM0_SC |= FTM_SC_TOF;                                               // clear interrupt
...
static __interrupt void _PWM_Interrupt_1(void)
{
    FTM1_SC |= FTM_SC_TOF;                                               // clear interrupt
...
static __interrupt void _PWM_Interrupt_2(void)
{
    FTM2_SC |= FTM_SC_TOF;                                               // clear interrupt
...
static __interrupt void _PWM_Interrupt_3(void)
{
    FTM3_SC |= FTM_SC_TOF;                                               // clear interrupt
    if (_PWM_TimerHandler[3] != 0) {                                     // if there is a user handler installed
...
#if defined _PWM_CONFIG_CODE
        {
    ...
    #elif (defined KINETIS_KL25 || defined KINETIS_KL26 || defined KINETIS_KL27) && defined TPM0_0_ON_D
    ...
    #elif (defined KINETIS_KL25 || defined KINETIS_KL26 || defined KINETIS_KL27) && defined TPM0_1_ON_D
                    _CONFIG_PERIPHERAL(D, 1, (PD_1_FTM0_CH1 | PORT_SRE_FAST | PORT_DSE_HIGH)); // FTM0_CH1 on PD.1 (alt. function 4)
    #elif (defined KINETIS_KL25 || defined KINETIS_KL26 || defined KINETIS_KL27) && defined TPM0_1_ON_E
        ...
        #elif (defined KINETIS_KL25 || defined KINETIS_KL26 || defined KINETIS_KL27) && defined TPM0_2_ON_D
                    _CONFIG_PERIPHERAL(D, 2, (PD_2_FTM0_CH2 | PORT_SRE_FAST | PORT_DSE_HIGH)); // FTM0_CH2 on PD.2 (alt. function 4)
        #elif (defined KINETIS_KL25 || defined KINETIS_KL26 || defined KINETIS_KL27) && defined TPM0_2_ON_E
        ...
        #elif (defined KINETIS_KL25 || defined KINETIS_KL26 || defined KINETIS_KL27) && defined TPM0_3_ON_D
                    _CONFIG_PERIPHERAL(D, 3, (PD_3_FTM0_CH3 | PORT_SRE_FAST | PORT_DSE_HIGH)); // FTM0_CH3 on PD.3 (alt. function 4)
        #elif (defined KINETIS_KL25 || defined KINETIS_KL26 || defined KINETIS_KL27) && defined TPM0_3_ON_E
                    _CONFIG_PERIPHERAL(E, 30, (PE_30_TPM0_CH3 | PORT_SRE_FAST | PORT_DSE_HIGH)); // TPM0_CH3 on PE.30 (alt. function 3)
        #elif (defined FTM0_3_ON_C && defined KINETIS_K64)
        ...
        #elif (defined KINETIS_KL25 || defined KINETIS_KL26 || defined KINETIS_KL27) && defined TPM0_4_ON_E
        ...
        #elif (defined KINETIS_KL25 || defined KINETIS_KL26 || defined KINETIS_KL27) && defined TPM1_1_ON_E
            ...
            if ((ulMode & FTM_SC_CPWMS) != 0) {                          // if center-aligned
                ptrFlexTimer->FTM_MOD = (ptrPWM_settings->pwm_frequency / 2); // set the PWM period - valid for all channels
                ptrFlexTimer->FTM_channel[ucChannel].FTM_CV = (ptrPWM_settings->pwm_value / 2); // set the duty cycle for the particular channel
            }
            else {
                ptrFlexTimer->FTM_MOD = (ptrPWM_settings->pwm_frequency - 1); // set the PWM period - valid for all channels
                ptrFlexTimer->FTM_channel[ucChannel].FTM_CV = ptrPWM_settings->pwm_value; // set the duty cycle for the particular channel            
            }
    #if !defined DEVICE_WITHOUT_DMA                                      // {1}
            if ((ulMode & (PWM_FULL_BUFFER_DMA | PWM_HALF_BUFFER_DMA)) != 0) { // if DMA is being specified
                fnConfigDMA_buffer(ptrPWM_settings->ucDmaChannel, ptrPWM_settings->ucDmaTriggerSource, ptrPWM_settings->ulPWM_buffer_length, ptrPWM_settings->ptrPWM_Buffer, (void *)&ptrFlexTimer->FTM_channel[ucChannel].FTM_CV, ((ulMode & PWM_FULL_BUFFER_DMA_AUTO_REPEAT) != 0), ((ulMode & PWM_HALF_BUFFER_DMA) != 0), ptrPWM_settings->dma_int_handler, ptrPWM_settings->dma_int_priority, 1); // source is the PWM buffer and destination is the PWM mark-space ratio register
            }
    #endif
            ulMode &= PWM_MODE_SETTINGS_MASK;                            // keep just the user's mode settings for the hardware
            if (ptrPWM_settings->int_handler != 0) {                     // {2} if an interrupt handler is specified it is called at each period
