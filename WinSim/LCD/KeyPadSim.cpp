...
extern int fnCheckKeypad(int x, int y, int iPressRelease)
{
    ...
    for (i = 0; i < (sizeof(user_buttons)/sizeof(USER_BUTTON)); i++) {
        if (iUserButtonStates[i] != 0) {                                 // originally pressed
            extern void fnInjectInputChange(unsigned long ulPortRef, unsigned long ulPortBit, int iState);
            iUserButtonStates[i] = 0;
            fnInjectInputChange(user_buttons[i].Port_Ref, user_buttons[i].Port_Bit, 0);
        }
    }
#endif
                ...
                if (user_buttons[i].button_area.top <= y) {
                    if (user_buttons[i].button_area.bottom >= y) {
                        ...
                        if (iPressRelease == 3) {                        // hovering over an input
                            fnSetLastPort(user_buttons[i].Port_Ref, user_buttons[i].Port_Bit); // {9} cause connected port details to be displayed
                            return 1;
                        }
                        if (iPressRelease != 0) {
                            ...
                            fnInjectInputChange(user_buttons[i].Port_Ref, user_buttons[i].Port_Bit, 1);
                        }
                        else {
                            iUserButtonStates[i] = 0;
                            fnInjectInputChange(user_buttons[i].Port_Ref, user_buttons[i].Port_Bit, 0);
